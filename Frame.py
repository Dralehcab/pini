...
class Frame(qt.QWidget):
    ...
    def endRegister(self):
        ...
        normVectorField = np.copy(np.sqrt(
            np.square(vectorField[:, :, :, 0] * px_z) + np.square(vectorField[:, :, :, 1] * px_x) + np.square(
                vectorField[:, :, :, 2] * px_y)))
    ...
    def macro5(self):
        ...
        pen_act = pg.mkPen((0, 200, 0, 255), width=1)
        self.p1 = self.plt.addPlot(row=0, col=0, title="Histogram",
                                   labels={'left': "Number of Vessels ", 'bottom': "Perimeters"})
        self.curve1 = self.p1.plot([], pen=pen_act)
    ...
    def macro3_GUI(self):
        ...
        msgBox = qt.QMessageBox(self)
        msgBox.setText(
            'Select the Region of Interest with the Polygon Tool. \n Select the 2 materials with Multiple Points')
        msgBox.exec_()
    ...
    def macro3(self):
        ...
        text = 'Adipose Tissue: Volume [mL]  :' + str(
            nbVoxelsFat * px_size[0] * px_size[1] * px_size[2] / 1000.0) + '\n'
    ...
    def macro1(self):
        ...
        if self.checkTrachea.checkState() != 2:
            ...
            if average_factor != 1:
                Contrast_Image = transform.resize(Contrast_Image, (
                Contrast_Image.shape[0], Contrast_Image.shape[1] / average_factor,
                Contrast_Image.shape[2] / average_factor))
                Tissue_Image = transform.resize(Tissue_Image, (
                Tissue_Image.shape[0], Tissue_Image.shape[1] / average_factor, Tissue_Image.shape[2] / average_factor))
                maskOut = transform.resize(maskOut, (
                maskOut.shape[0], maskOut.shape[1] / average_factor, maskOut.shape[2] / average_factor))
    ...
    def macro2(self):
        ...
        posZLast = int(np.max(np.where(maskSeg == 1)[0]) * 1.5)
    ...
    def macro2_GUI(self):
        ...
        self.VarInspiT = LabelEditAndButton(True, "Stdev Seg. Trachea Inspi", True, "0.9", False)
        self.VarExpiT = LabelEditAndButton(True, "Stdev Seg. Trachea Expi", True, "0.9", False)
    ...
    def endRegister2(self):
        ...
        self.text += 'Normal Recruitment [L] (Light Blue): ' + str(self.Vol_dense) + '\n'
        self.text += 'Injured Poor Ventilation [L] (Orange): ' + str(self.Vol_smallAirway) + '\n'
        self.text += 'Injured Emphysema [L] (Red): ' + str(self.Vol_emphysema) + '\n'
        self.text += '--------------------------------------------------------' + '\n'
    ...
    def _buttonSaveReg(self):
        ...
        outputFiles = [outputFolderFile + '/x.txt', outputFolderFile + '/y.txt', outputFolderFile + '/z.txt',
                       outputFolderFile + '/Inspi.txt', outputFolderFile + '/Expi.txt', outputFolderFile + '/DV.txt',
                       outputFolderFile + '/DHU.txt', outputFolderFile + '/Dist.txt',
                       outputFolderFile + '/Clusters.txt']
